...
int TextEditor::HighlightByExtension() {
    ...
    if  (!FileSuffix.isEmpty()) {
        ...
        // CSS files
        } else if (FileSuffix == "css"  ) {
            FileType = 4;

        // HTML files
        } else if ((FileSuffix == "htm")  | (FileSuffix == "html") | (FileSuffix == "xml")) {
            FileType = 5;

        // Java files
        } else if ((FileSuffix == "java") | (FileSuffix == "class") | (FileSuffix == "jar")) {
            FileType = 6;

        // Python files
        } else if  ((FileSuffix == "py")   | (FileSuffix == "pyw") | (FileSuffix == "pyc")  |
                  (FileSuffix == "pyo")  | (FileSuffix == "pyd") )  {
            FileType = 7;

        // LaTeX files
        } else if (FileSuffix == "tex"  ) {
            FileType = 8;

        // Vala files
        } else if (FileSuffix == "vala" ) {
            FileType = 9;
